...
async function exportLoveNotesForAudio() {
  try {
    console.log('🎵 Exporting love notes for audio generation...')
    ...
    if (!fs.existsSync(candidatesPath)) {
      ...
      console.log('Please run the love notes finder first')
      return
    }
    ...
    console.log(`📝 Found ${candidates.length} love notes`)
    
    // Create CSV content
    const csvHeader = 'id,text,date,emotion,filename\n'
    const csvRows = candidates.map((note, index) => {
      const filename = `love-note-${note.message_id}.wav`
    ...
    // Save CSV file
    const outputPath = path.join(process.cwd(), 'data', 'love-notes-for-audio.csv')
    fs.writeFileSync(outputPath, csvContent)
    
    console.log('✅ Exported love notes to:', outputPath)
    console.log('')
    console.log('📋 Next steps:')
    console.log('1. Go to https://app.hume.ai/')
    console.log('2. Upload the CSV file: data/love-notes-for-audio.csv')
    ...
    console.log('')
    console.log('🎵 After generating audio, run: node scripts/update-love-notes-audio.mjs')
    
  } catch (error) {
    console.error('❌ Error exporting love notes:', error)
  }
}

exportLoveNotesForAudio() 
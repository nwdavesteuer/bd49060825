#!/usr/bin/env python3
"""
Complete Nitzan Message Extractor
Extracts ALL messages between David and Nitzan from ALL contacts including:
- Email addresses (Gmail, iCloud)
- Phone numbers
- Group chats
"""
...
class CompleteNitzanExtractor:
    ...
    def find_all_nitzan_contacts(self):
        """Find ALL contacts that might be Nitzan."""
        try:
            conn = sqlite3.connect(self.messages_db_path)
            cursor = conn.cursor()
            
            # Find all contacts that could be Nitzan
            query = """
            SELECT DISTINCT 
                h.id,
                h.service,
    ...
    def find_all_nitzan_chats(self):
        """Find all chats involving Nitzan contacts."""
        try:
            conn = sqlite3.connect(self.messages_db_path)
            cursor = conn.cursor()
            
            # Find all chats that involve Nitzan contacts
            query = """
            SELECT DISTINCT 
                c.ROWID as chat_id,
                c.guid,
            ...
            WHERE h.id LIKE '%nitzan%' 
               ...
               OR h.id LIKE '%917%239%0518%'
               OR h.id = '+19172390518'
               OR c.chat_identifier LIKE '%nitzan%'
    ...
    def extract_all_nitzan_messages(self, output_file=None):
        """Extract ALL messages between David and Nitzan from ALL contacts."""
        print("🔍 Finding ALL Nitzan contacts...")
        
        contacts = self.find_all_nitzan_contacts()
        
        if not contacts:
            print("❌ No contacts found involving Nitzan")
            return None
        
        print(f"\n📋 Found {len(contacts)} contacts involving Nitzan:")
        print("-" * 80)
        print("Contact ID | Service | Messages | From David | To David")
        print("-" * 80)
        
        for contact in contacts:
            contact_id, service, total, from_david, to_david = contact
            print(f"{contact_id:30s} | {service:8s} | {total:8d} | {from_david:10d} | {to_david:8d}")
        
        print("\n🔍 Finding ALL Nitzan chats...")
        
        chats = self.find_all_nitzan_chats()
        ...
        export_data = {
            ...
            'contacts': [{'id': c[0], 'service': c[1], 'message_count': c[2], 'from_david': c[3], 'to_david': c[4]} for c in contacts],
...
def main():
    try:
        extractor = CompleteNitzanExtractor()
        
        # Generate output filename
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"complete_nitzan_messages_{timestamp}.json"
